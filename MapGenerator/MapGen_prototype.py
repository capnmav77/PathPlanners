...
class MapPartitioner:
    ...
    def recal_sector(self, start_x, start_y):
        ...
        if center_x < self.x and center_y < self.y:
            ...
            #     if center_x-1>0  :
            #         self.partitioned_map[i][center_x-1][center_y] = 7
            #     if center_x+1<self.x :
            #         self.partitioned_map[i][center_x+1][center_y] = 7
            #     if center_y-1 > 0 : 
            #         self.partitioned_map[i][center_x][center_y-1] = 7
            #     if center_y+1 < self.y :   
            #         self.partitioned_map[i][center_x][center_y+1] = 7
    ...
    def print_map(self):
        for map in self.partitioned_map:
            for row in map:
                print(row)
            print()

def get_map(x, y,z, num_elevators, num_obstacles):
